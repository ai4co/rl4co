...
class EvalBase:
    ...
    def __call__(self, policy, dataloader, **kwargs):
        ...
        return {"actions": actions.cpu(), "rewards": rewards.cpu(), "inference_time": inference_time, "avg_reward": rewards.cpu().mean()}
...
class GreedyMultiStartAugmentEval(EvalBase):
    ...
    def __init__(self, env, num_starts=None, num_augment=8,  force_dihedral_8=False, **kwargs):
        check_unused_kwargs(self, kwargs)
        super().__init__(env, kwargs.get("progress", True))
...
def evaluate_policy(env, policy, dataset, method="greedy", batch_size=None, max_batch_size=4096, start_batch_size=32_768,
                    auto_batch_size=True, save_results=False, save_fname='results.npz', **kwargs):

    num_loc = getattr(env, "num_loc", None)

    methods_mapping = {
        "greedy": {"func": GreedyEval, "kwargs": {}},
        "sampling": {"func": SamplingEval, "kwargs": {"samples": 100, "softmax_temp": 1.0}},
        "greedy_multistart": {"func": GreedyMultiStartEval, "kwargs": {"num_starts": num_loc}},
        "augment_dihedral_8": {"func": AugmentationEval, "kwargs": {"num_augment": 8, "force_dihedral_8": True}},
        "augment": {"func": AugmentationEval, "kwargs": {"num_augment": 8}},
        "greedy_multistart_augment_dihedral_8": {"func": GreedyMultiStartAugmentEval, "kwargs": {"num_augment": 8, "force_dihedral_8": True, "num_starts": num_loc}},
        "greedy_multistart_augment": {"func": GreedyMultiStartAugmentEval, "kwargs": {"num_augment": 8, "num_starts": num_loc}},
    }
    ...
    if auto_batch_size:
        ...
        batch_size = get_automatic_batch_size(eval_fn, max_batch_size=max_batch_size, start_batch_size=start_batch_size)
        print("Using automatic batch size: {}".format(batch_size))

    # Set up the dataloader
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=0, collate_fn=tensordict_collate_fn)
