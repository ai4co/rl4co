...
class SDVRPEnv(RL4COEnvBase):
    ...
    def _step(self, td: TensorDict) -> TensorDict:
        ...
        current_coord = torch.gather(
            td["observation"], 1, current_node.unsqueeze(-1).expand(-1, -1, 2)
        ).squeeze()
        ...
        current_demand = torch.min(
            torch.gather(demand, 1, current_node), available_capacity.squeeze(-1)
        )
        used_capacity = (td["used_capacity"] + current_demand) * (
            current_node != 0
        ).float()
        ...
        # Check finish
            ...
            unvisited_node.sum(-1) == 0, current_node.squeeze() == 0
        )
        ...
        return TensorDict(
            {
                ...
                "demands_with_depot": torch.cat(
                    (torch.zeros_like(demand[..., :1]), demand), -1
                ),
                "ids": demand,
                ...
                "unvisited_node": torch.ones(
                    (*batch_size, num_loc + 1), dtype=torch.bool, device=device
                ),
    ...
    def get_reward(self, td, actions) -> TensorDict:
        ...
        demand_with_depot = torch.cat(
            (torch.full_like(td["demand"][:, :1], -self.capacity), td["demand"]), 1
        )
        ...
        d = loc_with_depot.gather(
            1, actions[..., None].expand(*actions.size(), loc_with_depot.size(-1))
        )
    ...
    def generate_data(self, batch_size):
        ...
        demand = (
            torch.FloatTensor(*batch_size, self.num_loc).uniform_(0, 9).int() + 1
        ).float() / CAPACITIES[self.num_loc]
