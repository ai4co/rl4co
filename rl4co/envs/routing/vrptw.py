...
class VRPTWEnv(CVRPEnv):
    ...
    Args:
        ...
        max_time (float): maximum time for the environment. Defaults to 480.
    ...
    def __init__(
        self,
        max_loc: float = 1,  # different default value to CVRPEnv to match max_time, will be scaled
        max_time: float = 4.6,
        scale: bool = False,
        **kwargs,
    ):
    ...
    def generate_data(self, batch_size) -> TensorDict:
        ...
        durations = torch.rand(size=(*batch_size, self.num_loc + 1), dtype=torch.float32, device=self.device) * 0.05 +0.15
        # shape: (batch, problem)
        # range: (0.15,0.2) for T=4.6 

        node_lengthTW = torch.rand(size=(*batch_size, self.num_loc + 1), dtype=torch.float32, device=self.device) * 0.05 +0.15
        # shape: (batch, problem)
        # range: (0.15,0.2) for T=4.6 


        d0i = ((torch.cat((td["depot"][..., None, :], td["locs"]), -2) - td["depot"][..., None, :].expand(size=(*batch_size,self.num_loc + 1,2)))**2).sum(2).sqrt()
        # shape: (batch, problem)


        ei = torch.rand(size=(*batch_size, self.num_loc + 1), dtype=torch.float32, device=self.device).mul((torch.div((self.max_time*torch.ones(size=(*batch_size,self.num_loc + 1), dtype=torch.float32, device=self.device) - durations - node_lengthTW),d0i) - 1)-1)+1
        # shape: (batch, problem)
        # default velocity = 1.0

        min_times = ei.mul(d0i)
        # shape: (batch, problem)
        # default velocity = 1.0

        max_times = min_times + node_lengthTW
        # shape: (batch, problem)
