    ...
    def __init__(
        ...
        **kwargs
        ...
        if kwargs.get("demand_sampler", None) is not None:
            ...
            self.demand_sampler = get_sampler("demand", demand_distribution, min_demand, max_demand, **kwargs)
        
        # Capacity
        self.unnorm_capacity = kwargs.get("unnorm_capacity", None)
        if self.unnorm_capacity is None: # If not provided, use the default capacity from Kool et al. 2019
            self.unnorm_capacity = CAPACITIES.get(num_loc, None)
        if self.unnorm_capacity is None: # If not in the table keys, find the cloest number of nodes as the key
            cloest_num_loc = min(CAPACITIES.keys(), key=lambda x: abs(x - num_loc))
            self.unnorm_capacity = CAPACITIES[cloest_num_loc]
            log.warning(
                f"The unnorm capacity for {num_loc} locations is not defined. Using the closest capacity: {self.unnorm_capacity}\
        ...
        return TensorDict(
            {
                ...
                "depot": depot,
                "demand": demand / self.unnorm_capacity,
                "capacity": capacity,
            },
