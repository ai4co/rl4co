...
class PCTSPGenerator(Generator):
    ...
    def _generate(self, batch_size) -> TensorDict:
        ...
        # Sample penalty
        penalty = self.penalty_sampler.sample((*batch_size, self.num_loc + 1))

        # Sampel prize
        deterministic_prize = self.deterministic_prize_sampler.sample((*batch_size, self.num_loc + 1))
        stochastic_prize = self.stochastic_prize_sampler.sample((*batch_size, self.num_loc + 1))

        # Depot has no penalty and prize, set to 0 as placeholder
        penalty[:, 0] = 0.0
        deterministic_prize[:, 0] = 0.0
        stochastic_prize[:, 0] = 0.0
