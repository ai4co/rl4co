...
class MTVRPEnv(RL4COEnvBase):
    ...
    Features:
        - Capacity (C): Each vehicle has a maximum capacity $Q$, restricting the total load that can be in the vehicle at any point of the route.
        ...
        - Time Windows (TW): Every node $i$ has an associated time window $[e_i, l_i]$ during which service must commence.
        ...
        - Open Routes (O): Vehicles are not required to return to the depot after serving all customers.
        ...
        - Backhauls (B): Backhauls generalize demand to also account for return shipments. Customers are either linehaul or backhaul customers.
    ...
    Note:
        Please check out https://pyvrp.org/ for more information about VRP and its variants. Kudos to their help and great job!
