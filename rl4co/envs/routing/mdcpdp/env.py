...
class MDCPDPEnv(RL4COEnvBase):
    ...
    def _reset(self, td: Optional[TensorDict] = None, batch_size=None) -> TensorDict:
        ...
        num_loc = td["locs"].size(-2)
        num_depot = td["depot"].size(-2)
        ...
        to_deliver = torch.cat(
            [
                ...
                torch.zeros(
                    *batch_size, num_loc // 2, dtype=torch.bool, device=device
                ),
            ],
            dim=-1,
        )
        ...
        if self.start_mode == "random":
            current_depot = torch.randint(
                low=0, high=num_depot, size=(*batch_size, 1), device=device
            )
        ...
        current_length = torch.zeros((*batch_size, num_depot), dtype=torch.float32, device=device)

        # Arrive time for each city
        arrivetime_record = torch.zeros((*batch_size, num_loc + num_depot), dtype=torch.float32, device=device)

        # Cannot visit depot at first step # [0,1...1] so set not available
        available = torch.ones(
            (*batch_size, num_loc + num_depot), dtype=torch.bool, device=device
        )
