...
class VRPLEnv(RL4COEnvBase):
    ...
    Args:
        ...
        locations (cities) in the VRP, without the depot. (e.g. 10 means 10 locs + 1 depot)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.duration_limit = 3.0 if duration_limit is None else duration_limit
    ...
    def _step(self, td: TensorDict) -> TensorDict:
        ...
        self.selected_node_list = torch.cat((self.selected_node_list, current_node), dim=1)
        # shape: (batch, pomo, 0~)
        ...
        selected_xy = gather_by_index(
            td["locs"], current_node, squeeze=False
        )

        gathering_index_last = self.selected_node_list[:, -2][:,None,None].expand(-1,-1,2)
         
        last_xy =  gather_by_index(
            td["locs"], gathering_index_last, squeeze=False
        )

        selected_distance = ((selected_xy - last_xy)**2).sum(dim=2).sqrt()

        td["duration_limit"] -= selected_distance

        td["duration_limit"][current_node == 0] = self.duration_limit # refill length at the depot
    ...
    def _reset(
        ...
        self.selected_node_list = torch.zeros((*batch_size,1),dtype=torch.int64,device=self.device)
        ...
        td_reset = TensorDict(
            {
                ...
                "duration_limit": torch.full(
                    (*batch_size, 1), self.duration_limit, device=self.device
                ),
    ...
    def get_action_mask(td: TensorDict) -> torch.Tensor:
        ...
        if "action" not in td.keys():
            current_node =torch.zeros((td["locs"].shape[0],1),dtype=torch.int64, device=td["locs"].device)
        else:
            current_node = td["action"][:, None]  # Add dimension for step
        ...
        length_to_next  = ((selected_xy  - td["locs"])**2).sum(dim=2).sqrt()

        # length_to_next = ((selected_xy[:,None,:].expand(-1,-1,self.problem_size+1,-1) - xy_list)**2).sum(dim=3).sqrt()
        # # shape: (batch, pomo, problem+1)
        depot_xy = td["locs"][:,0,:]
        next_to_depot =  ((depot_xy[:,None,:].expand(td["locs"].shape)  - td["locs"])**2).sum(dim=2).sqrt()
        # shape: (batch, pomo, problem+1)

        length_too_small = td["duration_limit"] - 1E-6 < (length_to_next + next_to_depot )

        # Nodes that cannot be visited are already visited or too much demand to be served now

        mask_loc = (td["visited"][..., 1:].to(exceeds_cap.dtype) | exceeds_cap) | length_too_small[:,1:][:,None,:]

        # print(td["visited"][..., 1:].to(exceeds_cap.dtype).shape)
        # print(exceeds_cap)
        # print(length_too_small[:,1:][:,None,:])
        # print(mask_loc)
        # print(td["visited"][..., 1:][0])
        # print(td["visited"][..., 1:][-1])
        #input()
        # print(td["visited"][..., 1:].to(exceeds_cap.dtype).shape)
        ...
        #print(~torch.cat((mask_depot[..., None], mask_loc), -1).squeeze(-2))
    ...
    def generate_data(self, batch_size) -> TensorDict:
        ...
        # duration limit
        duration_limit = torch.full ((*batch_size,), self.duration_limit, device=self.device)
