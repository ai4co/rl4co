...
class OPEnv(RL4COEnvBase):
    ...
    def _step(self, td: TensorDict) -> TensorDict:
        ...
        used_capacity = (td["used_capacity"] + (current_coord - previous_coord).norm(p=2, dim=-1)) * ~(current_node == 0)
        ...
        feasible_actions = ~(visited | exceeds_length)
        visited[..., 0] = 1

        # We are done if run out the lenght capacity, i.e. no available node to visit
        done = visited.sum(-1) == visited.size(-1)

        feasible_actions[..., 0] = ~(current_node == 0).squeeze() | done
        ...
        return TensorDict(
            {
                "next": {
                    ...
                    "real_length_capacity": td["real_length_capacity"],
    ...
    def _reset(self, td: Optional[TensorDict] = None, batch_size=None) -> TensorDict:
        ...
        # Real length capacity
        real_length_capacity = torch.zeros((*batch_size, 1), device=device) + LENGTH_CAPACITY[self.num_loc]
        ...
        return TensorDict(
            {
                ...
                "prize": td["prize"],
                "used_capacity": used_capacity,
                "real_length_capacity": real_length_capacity,
