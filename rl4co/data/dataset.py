...
class TensorDictDataset(Dataset):
    ...
    def __init__(self, data):
        if isinstance(data, TensorDict):
            self.data = [{key: value[i] for key, value in data.items()} for i in range(data.shape[0])]
        else:
...
class TensorDictCollate(nn.Module):
    ...
    def forward(self, batch):
        ...
        return TensorDict({key: torch.stack([b[key] for b in batch]) for key in batch[0].keys()}, batch_size=len(batch))