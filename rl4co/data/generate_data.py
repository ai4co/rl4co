...
def generate_op_data(dataset_size, op_size, prize_type='const'):
    ...
    return {'depot': depot.astype(np.float32),
...
def generate_dataset(filename=None, data_dir='data', name=None, problem='all', data_distribution='all',
                     dataset_size=10000, graph_sizes=[20, 50, 100], overwrite=False, seed=1234, disable_warning=True):
    ...
    """
    assert filename is None or (len(problem) == 1 and len(graph_sizes) == 1), \
    ...
    for problem, distributions in problems.items():
        for distribution in distributions or [None]:
            for graph_size in graph_sizes:
                ...
                if filename is None:
                    fname = os.path.join(datadir, "{}{}{}_{}_seed{}.npz".format(
                        problem,
                ...
                if not overwrite and os.path.isfile(check_extension(fname, extension='.npz')):
                    if not disable_warning:
                        log.info("File {} already exists! Run with -f option to overwrite. Skipping...".format(fname))
                    continue
                ...
                log.info("Generating {} dataset and saving to {}".format(problem, fname))
