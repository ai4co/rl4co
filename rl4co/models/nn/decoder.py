...
class AutoregressiveDecoder(nn.Module):
    ...
    def __init__(self, env_name: str, embedding_dim: int, num_heads: int, **logit_attn_kwargs):
        super().__init__()
        ...
        self.context = env_context_embedding(self.env_name, {"embedding_dim": embedding_dim})
    ...
    def forward(
        ...
        # Construct environment if needed
        if isinstance(env, str):
            env_name = self.env_name if env is None else env
            env = ENV_REGISTRY[env_name]()
        elif isinstance(env, RL4COEnvBase):
            env = env
        else:
            raise ValueError(f"env must be either str or RL4COEnvBase, got {type(env)}")
