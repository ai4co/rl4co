...
class AttentionModel(PPO):
    def __init__(self, env, policy=None, critic=None):
        policy = PPOAttentionModelPolicy(env=env) if policy is None else policy
        critic = CriticNetwork(env=env) if critic is None else critic
        super(AttentionModel, self).__init__(env=env, policy=policy, critic=critic)
