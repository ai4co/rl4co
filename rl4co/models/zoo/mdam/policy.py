...
class AttentionModelPolicy(nn.Module):
    def __init__(
        ...
        **unused_kw
        ...
        self.context = [env_context(self.env.name, {"embedding_dim": embedding_dim}) for _ in range(num_paths)]

        self.project_node_embeddings = [nn.Linear(embedding_dim, 3 * embedding_dim, device=self.env.device, bias=False) for _ in range(num_paths)]
        self.project_node_embeddings = nn.ModuleList(self.project_node_embeddings)

        self.project_fixed_context = [nn.Linear(embedding_dim, embedding_dim, device=self.env.device, bias=False) for _ in range(num_paths)]
        self.project_fixed_context = nn.ModuleList(self.project_fixed_context)
        
        self.project_step_context = [nn.Linear(2 * embedding_dim, embedding_dim, device=self.env.device, bias=False) for _ in range(num_paths)]
        self.project_step_context = nn.ModuleList(self.project_step_context)

        self.project_out = [nn.Linear(embedding_dim, embedding_dim, device=self.env.device, bias=False) for _ in range(num_paths)]
            ...
            while not (self.shrink_size is None and td_list[i]["done"].all()):
                if j > 1 and j % self.eg_step_gap == 0:
                    if not self.is_vrp:
                        ...
                        mask_attn = mask
                    encoded_inputs, _ = self.embedder.change(attn, V, h_old, mask_attn, self.is_tsp)
                    fixed = self._precompute(encoded_inputs, path_index=i)
