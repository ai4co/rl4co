    ...
    for env_name in env_names:
        ...
        if os.path.exists(path):
            ...
            for exp in exps:
                ...
                checkpoint_folders = glob.glob(os.path.join(path, exp, '**', 'checkpoints'), recursive=True)
                ...
                if len(dates) > 1:
                    ...
                    print("Will default to the most recent one: {}".format(most_recent_date))

                # filter out files that do not contain the most recent date
                experiment_files = [file for file in experiment_files if most_recent_date in file]
