...
def test_routing(env_cls, batch_size=2, size=20):
    env = env_cls(generator_params={"num_loc": size})
...
def test_eda(env_cls, batch_size=2, max_decaps=5):
    env = env_cls(generator_params={"max_decaps": max_decaps})
    reward, td, actions = rollout(env, env.reset(batch_size=[batch_size]), random_policy)
    assert reward.shape == (batch_size,)
...
def test_smtwtp(env_cls, batch_size=2):
    env = env_cls(generator_params={"num_job": 4})
    reward, td, actions = rollout(env, env.reset(batch_size=[batch_size]), random_policy)
    assert reward.shape == (batch_size,)
