...
class SelfAttention(nn.Module):
    ...
    ---------
        ...
        attention_dropout: The dropout rate to apply to the attention
                           (default: 0.0)
    """
    def __init__(self, causal=False, softmax_scale=None, attention_dropout=0.0, _inf=-10000.0):
        super().__init__()
        self.causal = causal
        self.softmax_scale = softmax_scale
        self.dropout_p = attention_dropout
    ...
    def forward(self, qkv, causal=None, key_padding_mask=None):
        ...
        if key_padding_mask is not None:
            padding_mask = torch.full((batch_size, seqlen), self._inf, dtype=scores.dtype,
        ...
        if causal:
            ...
            causal_mask = torch.triu(torch.full((seqlen, seqlen), self._inf, device=scores.device), 1)
            # TD [2022-09-30]: Adding is faster than masked_fill_ (idk why, just better kernel I guess)
...
class CrossAttention(nn.Module):
    ...
    ---------
        ...
        attention_dropout: The dropout rate to apply to the attention
                           (default: 0.0)
    """
    def __init__(self, causal=False, softmax_scale=None, attention_dropout=0.0, neg_inf=-10000.0):
        super().__init__()
        self.causal = causal
        self.softmax_scale = softmax_scale
        self.dropout_p = attention_dropout
    ...
    def forward(self, q, kv, causal=None, key_padding_mask=None):
        ...
        scores = torch.einsum('bthd,bshd->bhts', q, k * softmax_scale)
        if key_padding_mask is not None:
            padding_mask = torch.full((batch_size, seqlen_k), self.neg_inf, dtype=scores.dtype,
                                      device=scores.device)
            padding_mask.masked_fill_(key_padding_mask, 0.0)
            # TD [2022-09-30]: Adding is faster than masked_fill_ (idk why, just better kernel I guess)
            scores = scores + rearrange(padding_mask, 'b s -> b 1 1 s')
        if causal:
            # "triu_tril_cuda_template" not implemented for 'BFloat16'
            # So we have to construct the mask in float
            causal_mask = torch.triu(torch.full((seqlen_q, seqlen_k), self.neg_inf,
