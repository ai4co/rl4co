...
class RolloutBaseline(REINFORCEBaseline):
    ...
    def epoch_callback(self, model, dl, epoch, env=None):
        """
    ...
    def rollout(self, model, dl, env=None):
        ...
        """     
        env_fn = lambda x: x if env is None else env.reset(init_observation=x['observation'])
        model.set_decode_type('greedy')
        with torch.no_grad():
            model.eval()
            device = next(model.parameters()).device # same as transfer_batch_to_device in lightning, may be cleaner but hey
            cost = [model(env_fn(batch.to(device)))["cost"] for batch in dl]
