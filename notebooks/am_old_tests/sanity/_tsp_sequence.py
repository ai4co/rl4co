...
class TSPEnv(EnvBase):
    ...
    def get_reward(loc, actions) -> TensorDict:
        ...
        # TODO: remake with torch.roll
        d = loc.gather(1, actions.unsqueeze(-1).expand_as(loc))
        # Length is distance (L2-norm of difference) from each next location from its prev and of last from first (with minus, since reward)
        return - ((d[:, 1:] - d[:, :-1]).norm(p=2, dim=2).sum(1) + (d[:, 0] - d[:, -1]).norm(p=2, dim=1))

    def get_costs(self, loc, actions) -> TensorDict:
        return -self.get_reward(loc, actions), None
    
    def _step(self, td: TensorDict) -> TensorDict:
        prev_a = td["action"]
        if batch_to_scalar(td["i"]) == 0:
            first_a = prev_a
            self.action_sequence = []
        else:
            first_a = td["first_a"]
        self.action_sequence.append(prev_a.detach().clone().squeeze())
        ...
        if done.any():
            # NOTE: use the function get_reward to calculate the reward for now, this gives a bit diffferent results (why?)
            action_sequence = torch.cat(self.action_sequence, dim=-1)[done]
            # Make sure that the action sequence is valid, else no reward
            if action_sequence.size(1) == self.n_loc:
                reward[done] = self.get_reward(td["loc"][done], action_sequence)
    ...
    def _reset(self, td: Optional[TensorDict] = None) -> TensorDict:
        ...
        loc = self.get_initial_conditions(n_loc, min_loc, max_loc, batch_size, self.rng)
    ...
    def get_initial_conditions(self, n_loc, min_loc=0, max_loc=1, batch_size=1, rng=None) -> torch.Tensor:
        return torch.rand((*batch_size, n_loc, 2), generator=rng) * (max_loc - min_loc) + min_loc
