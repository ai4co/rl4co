...
class WarmupBaseline(REINFORCEBaseline):
    ...
    def setup(self, model, dl):
        self.baseline.setup(model, dl)
...
class RolloutBaseline(REINFORCEBaseline):
    ...
    def setup(self, model, dl):
        self._update_model(model, dl)
        

    def _update_model(self, model, dl):
        device = next(model.parameters()).device
        self.model = copy.deepcopy(model).to(device)
        print("Evaluating baseline model on evaluation dataset")
        self.bl_vals = self.rollout(model, dl).cpu().numpy()
    ...
    def epoch_callback(self, model, dl, epoch):
        ...
        candidate_vals = self.rollout(model, dl).cpu().numpy()
    ...
    def rollout(self, model, dl):
        ...
        with torch.no_grad():
            ...
            cost = [model(batch.to(device))[0] for batch in dl]
        return torch.cat(cost, dim=0)