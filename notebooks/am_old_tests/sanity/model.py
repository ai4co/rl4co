...
from nets.graph_encoder2 import GraphAttentionEncoder2
...
from nets.mha import FlashCrossAttention, CrossAttention, FlashSelfAttention, SelfAttention
...
class AttentionModel2(nn.Module):
    ...
    def __init__(self,
        ...
        self.embedder = GraphAttentionEncoder2(
            n_heads=n_heads,
    ...
    def _one_to_many_logits(self, query, key, value, logit_K, mask):
        ...
        # q = query.view(batch_size, 1, self.n_heads, key_size)
        # k = key.view(batch_size, num_steps, 2, key_size, self.n_heads, key_size).permute(2, 0, 1, 3, 4)
        # v = value.view(batch_size, num_steps, self.n_heads, 1, val_size).permute(2, 0, 1, 3, 4)
        

        if not self._new_implementation:

            # Compute the glimpse, rearrange dimensions so the dimensions are (n_heads, batch_size, num_steps, 1, key_size)
            glimpse_Q = query.view(batch_size, num_steps, self.n_heads, 1, key_size).permute(2, 0, 1, 3, 4)
            
            # Batch matrix multiplication to compute compatibilities (n_heads, batch_size, num_steps, graph_size)
            compatibility = torch.matmul(glimpse_Q, key.transpose(-2, -1)) / math.sqrt(glimpse_Q.size(-1))
            if self.mask_inner:
            ...
            # q = rearrange(query, 'b 1 (h s) -> b 1 (h s)', h=self.n_heads)
            # k = rearrange(key, 'h b 1 g s  -> b g (h s)', h=self.n_heads)
            # v = rearrange(value, ' h b 1 g s -> b g (h s)', h=self.n_heads)  

            # # 1 means to keep, so we invert the mask
            # key_padding_mask = ~mask
            # heads = F.scaled_dot_product_attention(q, k, v, attn_mask=key_padding_mask)
            # heads = rearrange(heads, 'b 1 (h g) -> h b 1 1 g', h=self.n_heads)
