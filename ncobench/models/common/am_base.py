...
class AttentionModelBase(nn.Module):
    ...
    def forward(
        self, td: TensorDict, phase: str = "train", decode_type: str = "sampling"
    ) -> TensorDict:
        ...
        if (
            ...
            embeddings, _ = checkpoint(
                self.embedder, self._init_embed(td["observation"])
            )
        else:
        ...
        fixed = TensorDict(
            {
                ...
                "context_node_projected": self.project_fixed_context(graph_embed)[
                    :, None, :
                ],
    ...
    def _get_log_p(self, fixed, td, normalize=True):
        ...
        query = fixed["context_node_projected"] + self.project_step_context(
            self._get_parallel_step_context(fixed["node_embeddings"], td)
        )

        # Compute keys and values for the nodes
        glimpse_K, glimpse_V, logit_K = self._get_attention_node_data(
            fixed, td["observation"]
        )
        ...
        if (
            ...
            if td["i"][0].item() == 0:
                ...
                return embeddings.gather(
                    1,
                    torch.cat((td["first_a"], current_node), 1)[:, :, None].expand(
    ...
    def _one_to_many_logits(self, query, key, value, logit_K, mask):
        ...
        logits = torch.matmul(glimpse, logit_K.transpose(-2, -1)).squeeze(
            -2
